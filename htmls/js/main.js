...
(function ($) {
	...
	var Uknacks = {
		...
		stickyMenu: function () {
			// Headroom sticke menu - show stickymenu while scrolling top
			if (typeof Headroom === 'function') {
				var myElement = document.querySelector(".sticky-menu"),
		...
		twitterFeed: function () {
	        ...
	        if ($.fn.tweet && $('.twitter-feed').length) {
			    $('.twitter-feed').tweet({
			        modpath: 'js/twitter/',
			        avatar_size: 40,
					count: 8,
					query: 'google', // change username with query if you want to display search results
					loading_text:  'Searching twitter...',
			        join_text: '',
			        retweets: false,
			        template: '<header><figure>{avatar}</figure><h5>{name}</h5><a href="{user_url}" target="_blank">@{screen_name}</a></header><a class="twitter-icon" href="{tweet_url}" target="_blank"></a><div class="tweet-content">{text}</div>',
			    });
